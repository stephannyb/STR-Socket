...
class Game(arcade.Window):
    def __init__(self, width, height):
        ...
        self.t_a.start()
        self.t_z.start()
        self.veltrens = threading.Thread(target=self.update_vel).start()
    ...
    def update_vel(self):
        while 1:
            ...
            cmd = str(modifiedMessage)
            self.modifica_velocidade(cmd)

    def modifica_velocidade(self, cmd):
        print(cmd)
        if cmd == "b'vel_+_verde'" and (self.verde_vel_x < 10 or self.verde_vel_y < 10):
            self.verde_vel_x += 0.25
            self.verde_vel_y += 0.25

        elif cmd == "b'vel_+_roxo'" and (self.roxo_vel_x < 10 or self.roxo_vel_y < 10):
             self.roxo_vel_x += 0.25
             self.roxo_vel_y += 0.25

        elif cmd == "b'vel_+_amarelo'" and (self.amarelo_vel_x < 10 or self.amarelo_vel_y < 10):
             self.amarelo_vel_x += 0.25
             self.amarelo_vel_y += 0.25

        elif cmd == "b'vel_+_azul'" and (self.azul_vel_x < 10 or self.azul_vel_y < 10):
             self.azul_vel_x += 0.25
             self.azul_vel_y += 0.25

        elif cmd == "b'vel_-_verde'" and (self.verde_vel_x > -1 or self.verde_vel_y > -1):
             self.verde_vel_x -= 0.25
             self.verde_vel_y -= 0.25

        elif cmd == "b'vel_-_roxo'" and (self.roxo_vel_x > -1 or self.roxo_vel_y > -1):
             self.roxo_vel_x -= 0.25
             self.roxo_vel_y -= 0.25

        elif cmd == "b'vel_-_amarelo'" and (self.amarelo_vel_x > -1 or self.amarelo_vel_y > -1):
             self.amarelo_vel_x -= 0.25
             self.amarelo_vel_y -= 0.25

        elif cmd == "b'vel_-_azul'" and (self.azul_vel_x > -1 or self.azul_vel_y > -1):
             self.azul_vel_x -= 0.25
             self.azul_vel_y -= 0.25
